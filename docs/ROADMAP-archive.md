# VibeCoach Roadmap — Archief (afgeronde epics)

Volledige historie van afgeronde epics (✅). De **levende** planning — open & actief werk + de "Open werk"-index — staat in **[ROADMAP.md](ROADMAP.md)**. Dit bestand groeit append-only en wordt alleen op aanvraag gelezen.
...
**Gemerged via PR #312. Effort:** ~2–4u. **Out of scope:** datamodel-uitbreiding (extra gevoel-axis), terugschrijven naar HealthKit, historische check-ins migreren. **Open punt:** definitieve emoji/labels kunnen na on-device review nog schuiven.
...
#### ✅ Backlog-item afgerond elders: meerdaagse events first-class

Dit idee is volledig uitgewerkt en geleverd als **Epic #55** (Meerdaagse events first-class) + **Epic #56** (locatie-bewust per-etappe-weer) — zie het [archief](ROADMAP-archive.md). Hier alleen als kruisverwijzing bewaard.
...
**Status:** ✅ — afgesloten juni 2026 (zie kop): 37.1–37.6 gemerged (PR #291–#306). De resterende i18n-polish — 37.7 (doc-files zélf naar Engels; ROADMAP blijft bewust NL als werktaal), 37.8 (per-taal UI-test-pass) en native DE/ES-vertaalreview — is bewust niet uitgevoerd en geen blocker.
...
* **→ 39.3 — verplaatst naar Epic #62 (story 62.6).** Project-instelling "Strict Concurrency Checking" naar `Complete`: optioneel, dwingt af dat toekomstige PR's geen nieuwe regressies introduceren. Losgemaakt van deze cleanup omdat het mogelijk nieuwe Sendable-warnings boven water haalt; opgepakt als losse PR onder Epic #62.
...
**Status:** ✅ — kern (39.1 + 39.2) live en gemerged. De optionele build-setting-promotie (39.3) is doorgeschoven naar **Epic #62** (resterende hardening).
...
**Status:** ✅ — gemerged via PR #260. 743 unit-tests groen; on-device gevalideerd.
...
**Status:** ✅ — gemerged via PR #261 (+ fix PR #262: `.skippedSameSource`-pad doet ook de weer-merge). 769 unit-tests groen; on-device gevalideerd.
...
* **→ 51.B — verplaatst naar Epic #62 (story 62.1).** Doelen aanmaken & beheren: datum minstens +7 dagen, realistische stretch-tijden per sport, titel-trim, soft-delete tegen stale coach-context.
...
* **→ 51.D — verplaatst naar Epic #62 (story 62.2).** AI-provider & API-sleutel: auto-trim bij plakken, prefix-detectie voor verkeerde provider, test-feedback persistent.
* **→ 51.E — verplaatst naar Epic #62 (story 62.3).** Onboarding & toestemmingen: HealthKit als vereist, notificaties als optioneel, status-banner bij skip, achteraf-intrekken-detectie, status-overzicht in Settings.
* **✅ 51.F — Data syncen:**
  ...
  - **→ F3 + F4 + F6 — verplaatst naar Epic #62 (story 62.4):** HK per-type permissie, weer-fout non-blocking met retry-marker, captive-portal-detectie.
* **→ 51.G — verplaatst naar Epic #62 (story 62.5).** Proactieve coach (achtergrond): status-rij in Settings, notificatie-permissie pre-check, registratie-fout zichtbaar.
...
**Status:** ✅ — geleverde stories (A, C, F1/F2/F5, H) gemerged via PR #267/#268/#269/#270/#272. De resterende open hardening-groepen (B, D, E, F3/F4/F6, G) zijn niet vervallen maar samengebracht onder **Epic #62** zodat ze als één vooruitkijkend doel zichtbaar blijven.
...
### ✅ Epic #58: README als showcase — gebruiker-gerichte productpagina

De `README.md` was **ontwikkelaar-gericht** (status, tech stack, setup); een nieuwe bezoeker kreeg geen gevoel van *wat de app voor hém doet* of *hoe die eruitziet*. De README is herschreven naar een echte productpagina: bovenaan een hero + acht feature-blokken (screenshot + nuchtere omschrijving + "what you get"-voordeel), daarna een beknopte techniek-samenvatting ("Under the hood") met doorverwijzingen i.p.v. duplicatie, en als laatste een complete "Build & run it yourself"-sectie.

**Afgestemd (juni 2026):** volledige showcase **in de README zelf** (niet als aparte pagina) → §7 is bijgesteld zodat alleen het techniek-samenvattingsblok < 1 screen moet blijven, de showcase mag langer. Screenshots als **placeholders** in `docs/screenshots/` (vaste bestandsnamen, de maintainer swapt later echte captures). Toon **nuchter/feitelijk**, tekst in **Engels** (consistent met §7/§10 docs-conventie).

* **✅ Story 58.1 — Showcase-herstructurering.** README herschreven: hero (`00-hero.png`) + "Why VibeCoach" + acht feature-blokken met links/rechts-uitgelijnde screenshots (Vibe Score, proactieve coaching, AI-coach BYOK, workout-deep-dive, doel-fases, meerdaagse events + per-etappe-weer, dual-source sync, taal & privacy). Daarna "Under the hood" (stack-tabel + principes + CI + recently-completed + doorverwijzingen naar ARCHITECTURE/ROADMAP/CLAUDE/architecture.html) en "Build & run it yourself" (vereisten, `Secrets.swift`, BYOK-sleutel in Settings, optionele Strava-proxy, build & run, tests draaien). De simulator-push-test staat nu in een ingeklapte "Developer notes".
* **✅ Story 58.2 — Screenshot-infrastructuur.** `docs/screenshots/` met negen placeholder-PNG's (320×690, stdlib-gegenereerd) + een `README.md` die per slot uitlegt welke capture erbij hoort, het formaat (PNG portrait, EN, demo-data), en hoe je ze swapt zonder de hoofd-README aan te raken.
* **✅ Story 58.3 — Doc-discipline bijwerken.** `CLAUDE.md` §7: de README-scoperegel verzoend met de showcase (showcase mag lang, techniek-samenvatting < 1 screen) + het README-update-protocol verwijst nu naar "feature block + screenshot-slot" voor significante features.
...
* **✅ Story 60.1 — Service + value-types + venster-unificatie.** `PhaseTimeline`/`PhaseSummary`/`PhaseTarget`/`PhaseMilestone`/`PhaseWindow`/`PhaseStatus` (`Models/PhaseTimeline.swift`) + `PhaseWindowCalculator` als enige bron voor fase-vensters (week-budget, consistent met de balk). `ProgressService.phaseTimeline(for:activities:now:)` bouwt alle vier fases met targets (langste sessie + wekelijkse TRIMP) en bucket de blueprint-mijlpalen per fase op deadline; blueprint-loze doelen krijgen een generieke TRIMP-target. 9 unit tests in `PhaseTimelineTests` (vensters, contiguïteit, korte-doel-compressie, status, bucketing, taper-inversie, future-nil, fallback, logged-activity). *Scope-noot:* de coach-pad `BlueprintGap.currentPhase` (via `TrainingPhase.calculate`) is bewust niet aangeraakt — alleen de view-laag (balk + lijst) is geünificeerd; volledige unificatie van het prompt-pad is een mogelijke vervolgstap.
* **✅ Story 60.2 — UI `PhaseMilestonesView`.** `DisclosureGroup` per fase (eerste in de codebase), huidige fase default open; dicht = fasekleur-dot + naam + datumbereik + status, open = target-rijen (voortgangsbalk, taper "(max)") + mijlpaal-rijen (streefdatum + vinkje). Geïntegreerd in `GoalsListView` i.p.v. de oude vier-punts-timeline; `phaseSegments` put nu uit dezelfde `PhaseWindowCalculator` (balk + lijst kunnen niet meer divergeren). Dode code verwijderd (`milestonesSection`, `milestonesForGoal`, `GoalMilestoneItem`, `MilestoneTimelineRow`).
* **✅ Story 60.3 — i18n.** Catalog-keys NL/EN/DE/ES voor de nieuwe labels + de format-key `Streefdatum %@`; "Nu"/"Langste sessie" hergebruikt; fasenamen blijven Engels (`displayName`). Geverifieerd via de gecompileerde `.lproj` (en/de/es), incl. de `%@`-key (§13). On-device validatie staat in de PR-checklist.
...
* **✅ Story 61.1 — Log-hygiëne (§11) afdwingen.** Elke release-bereikbare `print()` in `Services/`/`Models/`/`ViewModels/`/app-entry vervangen door een `AppLoggers`-call met expliciete `privacy:`-modifier; de volledige coach-prompt + ruwe model-respons worden niet meer verbatim gelogd (alleen een niet-identificerend lengte-signaal). De PHI-dumps in de View-laag (rauwe DB-dump, Vibe Score met HRV/slaap, leeftijd/BMR) verwijderd; foutpaden routeren via `AppLoggers` (nieuwe categorieën `coach`/`trainingPlan`/`notifications`/`deepSync`/`workoutSamples`/`dashboard`). Losse `Logger`-instanties (DeepSync, WorkoutSamples) gecentraliseerd in `AppLoggers`; workout-UUID's/activity-id's nu `privacy: .private`. Resterende `print()`s zitten achter `#if DEBUG` en shippen niet.
* **✅ Story 61.2 — Credential-opslag device-only maken.** `KeychainService` gebruikt nu `kSecAttrAccessibleWhenUnlockedThisDeviceOnly` voor alle tokens + sleutels — vereist een ontgrendeld toestel én blijft op het apparaat (uit back-ups en toestel-herstel). De waarden zijn opnieuw af te leiden bij een schone install.
* **✅ Story 61.3 — Gezondheidsdata in rust beschermen.** Expliciete file-protection-klasse (`NSFileProtectionCompleteUnlessOpen`) op de SwiftData-store + WAL/SHM-sidecars na succesvolle container-init (normaal- én fresh-DB-fallback-pad), best-effort per §12. Nieuwe pure helper `PHIContextCache` (UserDefaults-geïnjecteerd, §6) wist alle cleartext PHI-context-caches + `WorkoutInsightCache` bij bron-ontkoppeling/uitloggen (M-2-minimum + L-9); 4 unit tests. Volledige verplaatsing van de caches naar de beschermde store is een vervolgstap (backlog hieronder).
* **✅ Story 61.4 — Veiligheidsvalidatie rond de AI-coach.** Nieuwe pure helper `TrainingPlanSafetyValidator` (§6) begrenst door het model voorgestelde parameters — sessieduur (≤600 min) en target-TRIMP (≤1000, negatief→0) — bij het enige `updatePlan`-chokepoint, vóór opslag/weergave en los van de prompt (L-2). Nieuwe pure helper `PromptInputSanitizer` saneert extern-gesynchroniseerde vrije tekst (Strava `activity.name`): control chars/newlines weg, whitespace ingedikt, lengte gecapt, placeholder bij leeg — toegepast op de prompt-interpolatie (L-1). 13 unit tests. Sluit aan op de fysiologische guardrails uit Epic 13.
* **✅ Story 61.5 — Build- & robustness-hygiëne.** Test-only `-UITesting`-bypasses in `ChatView` + `AddGoalView` achter `#if DEBUG` gezet (kunnen niet meer in een release-binary bestaan, L-6). Expliciete `timeoutInterval = 30` op alle Strava-requests (incl. token-refresh) en de live/per-etappe weer-requests (L-4; `HistoricalWeatherService` houdt bewust de default via z'n injecteerbare fetcher-seam). Strava-paginatie krijgt een harde page-cap (`maxPages = 50`, gelogd bij bereiken) en de server-gestuurde `Retry-After` wordt geklemd op max 24u (I-8). 3 nieuwe clamp-tests; bestaande Strava-tests groen. *De SDK-migratie is afgesplitst als Story 61.8 (grotere, aparte ingreep).*
* **✅ Story 61.6 — Locatie-privacy consistent maken.** Nieuwe pure helper `CoordinatePrivacy` (§6) als enige bron voor coördinaat-afronding (0.1° / ~11km). `WeatherManager` (live forecast) en `OpenMeteoForecastClient` (per-etappe) ronden nu áf vóór de request; `HistoricalWeatherService.roundForPrivacy` delegeert ernaar. De privacy-marge hangt niet meer af van de losse `desiredAccuracy`-instelling (L-5). 5 unit tests (+ bestaande weer-tests blijven groen).
* **✅ Story 61.7 — PHI-context-caches naar beschermde opslag (vervolg op 61.3).** Nieuw `@Model CoachContextCache` (SwiftData-singleton, SchemaV6) vervangt 17 `@AppStorage`-properties in `ChatViewModel` — HRV/slaap-scores, per-lichaamsgebied pijnscores, 14-daagse workout-TRIMP/HR-geschiedenis, voedingsdata e.d. — die als plaintext in `Library/Preferences` stonden. Door ze naar de file-protected SwiftData-store te verplaatsen (lightweight migratie V5→V6) genieten ze `NSFileProtectionCompleteUnlessOpen` zoals alle andere `@Model`-types. `PHIContextCache` beheert nu alleen de resterende UserDefaults-sleutels; `StravaAuthService.logout(modelContext:)` wist beide kanten. `AppTabHostView` injecteert de `ModelContext` via `chatViewModel.configure(with:)`. 3 migratietests (file-backed, §2.1). **Gemerged via PR #320. Bugfix PR #322:** `DashboardView` las `lastAnalysisTimestamp` nog van de niet-meer-bijgewerkte `@AppStorage`-spiegel; nu direct van `viewModel.lastAnalysisTimestamp`.
* **✅ Story 61.8 — Migratie weg van de niet-onderhouden AI-SDK (afgesplitst uit 61.5).** Vervangt `google/generative-ai-swift` (deprecated upstream, CWE-1104/L-3) door `GeminiRestClient` — een directe URLSession REST-client tegen de Gemini v1beta `/generateContent`-endpoint, hetzelfde patroon als de OpenAI/Anthropic/Mistral-clients. Auth via `x-goog-api-key`-header (niet query-param, zodat de sleutel uit access-logs blijft). Het enige externe Swift-pakket is daarmee verwijderd. `GenerateContentError`-tak verwijderd uit mapper, validator en `isOverload()`. **Gemerged via PR #321.**

**Status 61.1–61.3:** ✅ PR #316. **Status 61.4:** ✅ PR #317. **Status 61.6:** ✅ PR #318. **Status 61.5:** ✅ PR #319. **Status 61.7:** ✅ PR #320 + bugfix PR #322. **Status 61.8:** ✅ PR #321. **Epic #61 volledig afgerond.**

**Pickup-trigger:** Epic #61 (security hardening) volledig afgerond. Alle acht stories gemerged. Volgende prioriteit: **Epic #62** (resterende user-feedback hardening) of zie backlog.
