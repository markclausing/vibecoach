...
### ✅ Epic #67: "How this project is built" — development-workflow visualisation in the architecture viewer

Maintainer goal (2026-07-03, verbatim): *"make the technical workflow of collaborating with the developer agent, the technical workflow of CI, and the agreements with the agent about branching and documentation all more visual in the architecture HTML."* Audience: non-programmers and semi-technical colleagues the maintainer wants to show *how* the app is built. Builds directly on the Epic #66 viewer (three-level progressive disclosure) by adding a fourth story — the development process itself — respecting the §7 derivation chain (explanation lives in `ARCHITECTURE.md` first, mirrors into `architecture.json`, then renders in `architecture.html`; JSON re-injected via the §7 snippet, round-trip a no-op).

**What shipped:** the **Reference** tab was renamed **"🛠️ How it's built"** and now leads with four diagram-first, plain-language blocks (data-driven from a new `development` object in `architecture.json`), with the pre-existing testing/agent reference material folded in below as "Go deeper":

1. **The collaboration loop** — a colour-coded step flow with two actors (🧑 maintainer follows the theme colour / 🤖 agent a fixed purple accent, so the split reads without relying on colour alone): intent → epic on the roadmap → agent implements (branch + tests + docs) → PR → CI verdict → maintainer tests on device → squash & merge → archive & clean up, with a "↻ repeats every epic" note.
2. **The CI pipeline** — an inline-SVG dependency **DAG** (level = longest path from a root; SwiftLint ∥ Unit Tests → UI Tests → Coverage, with arrowheads showing what waits for what and the CodeQL pair as a parallel track), a per-job plain-English legend, plus a release strip (build-number-per-commit vs. merge-the-Release-PR marketing version) and a semver bump table (`fix:`→patch / `feat:`→minor / `feat!`→major / `docs:`→none).
3. **Branching & PR agreements** — an icon + monospace-pattern legend for the seven branch types, the four core rules (one-fix-per-PR, PR-title-is-the-Conventional-Commit, docs-only-straight-to-main, agent-opens-maintainer-merges), and a highlighted **stacked-PR inset** rendering the merge-bottom-up + delete-after-merge lesson visually.
4. **The documentation system** — file cards for the 4 source-of-truth files + 2 derived artefacts (each with a who-reads-this badge), a `json → injected into → html` sync note, the §7 Definition-of-Done checklist as a tap-to-expand `<details>`, and a ⏳→🔄→✅→📦 roadmap-to-archive flow.

**Stories** (all delivered in this one PR per the maintainer's single-PR flow for a self-contained docs change):

* **✅ 67.1 — ARCHITECTURE.md section + JSON data model.** New `## 19. Development workflow` section in `ARCHITECTURE.md` (~1 screen: collaboration loop, CI/release pipeline, branch/PR conventions, documentation system — referencing CLAUDE.md §7/§8 + §11 as the normative source, not duplicating them). Mirrored structurally in a new top-level `development` object in `architecture.json` (`collaboration` / `ci` / `branching` / `documentation`), so future agents update the data machine-readably; `meta.docRevision` bumped 36 → 37.
* **✅ 67.2 — "How it's built" view with the four diagrams.** Renamed the Reference tab (nav stays at 5 items), rendered the four blocks in pure inline HTML/CSS/SVG (no external libraries — the self-contained contract), reusing Epic #66's visual language (green identity, section heads, term-popover chips for jargon: PR, squash, DAG, Conventional Commits, semver, CI, main, release-please). Mobile-first: diagrams scroll inside their own `overflow-x:auto` containers, never the body; tap targets ≥44px. All steps/jobs/types/files render from the JSON `development` object.
...
Maintainer goal (July 2026): a small chat on the workout detail page where the user can talk *about that workout* — how it felt, the route, their condition that day/week. The app remembers this locally and feeds it into plans and feedback, with guardrails keeping the chat on-topic. Design decisions (maintainer): **hybrid persistence** (thread per workout + distilled facts), **inline section** at the bottom of `WorkoutAnalysisView`, fact management **at the workout itself only**.

* **✅ 70.1 — SchemaV7: two local-only `@Model`s + migration.** `WorkoutChatEntry` (thread; `activityID` keyed by value to `ActivityRecord.id` — no relationship, activities re-sync while chat is local-only) and `WorkoutChatFact` (+ type-safe `WorkoutFactCategory`: feel/route/dayCondition). Pure addition per §2.1: `SchemaV7`, lightweight V6→V7 stage, container bump, file-backed `SchemaMigrationV6ToV7Tests` (goal/preference/activity survival + new-model writability).
* **✅ 70.2 — `WorkoutChatScopeInstruction` guardrail.** Narrower sibling of `ChatScopeInstruction`: scope = this workout + the user's condition today/this week; everything else (incl. plan questions) redirects to the Coach tab. Carries the `{"reply","workoutFacts"}` JSON contract; category literals shared with the enum and guarded by tests on both sides.
* **✅ 70.3 — Parser + SwiftData-free view model.** `WorkoutChatResponse` (defensive decode), `WorkoutChatResponseParser` (reuses `extractCleanJSON`; unknown category dropped at the front door; non-JSON degrades to reply-only) and `WorkoutChatViewModel` (callback-based persistence per the `onNewPreferencesDetected` precedent; overload waterfall via `CoachModelProvider`, which gained an injectable systemInstruction builder — default behaviour unchanged, 18/18 `ChatViewModelTests` green).
* **✅ 70.4 — Inline "Discuss this workout" UI.** `WorkoutChatSection` owns the SwiftData side (@Query thread + facts, fresh-fetch containment dedupe); fact chips with ✕ as the single management surface; collapsed thread with expander; reused `MessageBubble` + `ChatInputValidator`; `-UITesting` key-gate bypass per §6; `UITestMockGenerativeModel` answers workout-chat prompts via the `[WORKOUT DATA]` marker. 7 new catalog keys (NL + EN/DE/ES), normalized, compiled `.lproj` verified.
...
* **✅ 72.6 — Personalised weekly-volume ramp (on-device feedback).** `Services/WeeklyVolumeRamp.swift`: the gap analysis' cumulative km/TRIMP targets no longer multiply the static blueprint volume by phase weeks ("9 / 121 km" in a 2-week base) but ramp linearly from the athlete's trailing 4-week average at goal creation toward the blueprint volume at taper start (taper ×0.6); no recent data → legacy behaviour. 12 unit tests.
...
**Problem.** Periodisation was computed **per goal, fully independently**: `PeriodizationEngine.evaluate` derived the phase purely from that one goal's `weeksRemaining`, and `evaluateAllGoals` mapped each goal in isolation — so every goal ran its own base→build→peak→**taper**. With two overlapping race goals that physically contradicts: the maintainer had *Halve marathon Haarlem* (64 d) and *Marathon Amsterdam* (86 d); Haarlem's own taper told the athlete to cut load exactly when Amsterdam still needed to build. The dashboard weekly target was wrong too — `DashboardView.weeklyTRIMPTarget` just took `.max()` across goals — and the coach prompt carried two contradicting `═══ PERIODISERING ═══` sections.

**Product model (locked with maintainer, July 2026):** explicit A/B/C race priority per goal with **exactly one A-race** anchoring the macrocycle (default when unset: latest target date = A); **every non-A interim race gets a mini-taper** (~4 days' unload) after which the build toward the A-race resumes; **one program timeline**, **one combined weekly TRIMP target**, **one coach periodisation context**.

* **✅ 73.1 — `MacrocyclePlanner` engine (pure Swift, no schema).** `Services/MacrocyclePlanner.swift` + `Models/UnifiedProgram.swift` (`RacePriority`, `RaceMarker`, `UnifiedProgram`). Anchor selection (explicit A beats the date-derived default), macrocycle phase windows to the anchor via `PhaseWindowCalculator` (base spans from the earliest goal `createdAt`), interim races overlaid with a ~4-day mini-taper window, effective current phase (mini-taper overrides the underlying window) and the combined weekly TRIMP target. AppStorage-free (§6).
* **✅ 73.2 — `racePriority` field + SchemaV8 migration.** `FitnessGoal.racePriority: RacePriority?` (A/B/C, `String, Codable`); SchemaV7 snapshot + `MigrationStage.lightweight` V7→V8, container bumped to `SchemaV8.models`, file-backed `SchemaMigrationV7ToV8Tests`. A/B/C picker in `AddGoalView`/`EditGoalView` with catalog entries. The manual priority overrides the date-derived default in the planner.
* **✅ 73.3 — Unified weekly target + current-phase wiring.** `DashboardView.weeklyTRIMPTarget` `.max()` → `UnifiedProgram.weeklyTrimpTarget` (feeding the "behind on plan" banner and `WeekTimelineView`); `DashboardHeaderView` reads the macrocycle phase + program week (mini-taper labelled explicitly); `atRiskGoals` judges every goal against the effective phase, so an interim race is no longer flagged for "tapering overload" while the program wants build volume. `UnifiedProgram.programWeek(at:)` added (pure, clamped).
* **✅ 73.4 — One unified coach periodisation context.** New `MacrocycleContextFormatter`: one macrocycle header (🎯 A-RACE + program week, the phase applying to every goal, the combined weekly target, interim races as tune-ups with their mini-taper date, ⚡ MINI-TAPER ACTIVE, and a HARD RULE against a full taper for an interim race) above per-goal blocks that `PeriodizationEngine`'s new `phaseOverride:` has evaluated with the same phase. Both emitters (chat coach + workout analysis) go through it; `🎯 A-RACE` / `⚡ MINI-TAPER` registered in `structuralPromptMarkers` with a matching systemInstruction rule (§13).
* **✅ 73.5 — Program-timeline UI (Goals tab).** New `Views/Goals/ProgramTimelineCard.swift`: one macrocycle bar to the A-race (phase segments sized by real duration, interim races as mini-taper flag markers, a "you are here" tick), current phase + combined weekly target + next-phase hint, and a race list with A/B/C badges (`RacePriorityBadge`). `GoalHeroCard` gained `showsPhaseBar` + `racePriority`, so the competing per-goal bars are gone and the A/B/C chip takes their place. `UnifiedProgram.fraction(of:)` positions the markers.
* **✅ 73.6 — Single-goal + degenerate-case parity.** A lone goal's macrocycle windows equal `PhaseWindowCalculator.windows(for:)` exactly; a blueprint-less goal still anchors a program with a usable target; completed/expired goals never reach the timeline nor stretch its span; same-date races yield one anchor and no mini-taper; a goal created today with a race 3 days out still plans. The empty state is untouched.

**Merged via PR #356. Effort:** ~1 session (6 stories; 30 new unit tests, unit suite 1275 green bar the known simulator Keychain entitlement failures, UI suite 26/26).

**Gotcha worth remembering (dev workflow only):** the V7→V8 migration was verified safe by seeding real V5/V6/V7 stores from a `main` worktree and migrating them, but installing a **pre-73 (SchemaV7) build on a device that already ran a V8 build** trips "Cannot use staged migration with an unknown model version" → the defensive `makeModelContainer` fallback wipes and rebuilds an empty store. Production users never downgrade, so App Store users are unaffected.

---

## ✅ Epic #74: Session load — RPE measures intensity, not strain

**Trigger (maintainer report, 20 Sept 2026):** *"Ik kies steeds de verkeerde RPE na een training. Gisteren had ik 30 km hardgelopen en ik moet nu echt goed herstellen. Maar ik koos 'ging prima: ik kon blijven praten', wat ook klopt tijdens de training. Maar de opgebouwde vermoeidheid is wel erg hoog."*

The rating was not wrong — the instrument was too narrow. The check-in asked a **talk test** ("Kon makkelijk doorpraten"), which measures intensity *during* the effort, and the app then treated that single number as the session's strain. Strain is intensity × time, so a long endurance run at a conversational pace scores "easy" while being the heaviest stimulus of the month. Worse, the `systemInstruction` had a rule reading exactly that pattern the wrong way round — *"If RPE is low (1-4) while TRIMP is high: the athlete is having a good day"* — turning the session that needs recovery into a green light for more volume.

Both halves are fixed, deliberately redundantly: the copy helps the user rate the session better, and the derived load means the coach is right even when they don't. Architecture rationale (formula, band cut-offs, emitters) → [`ARCHITECTURE.md` §22](ARCHITECTURE.md#22-session-load--why-rpe-alone-misleads-the-coach-epic-74).

**Stories:**

* **✅ 74.1 — `SessionLoadCalculator` (Foster sRPE).** New pure-Swift helper in `Services/Physiology/`: `load (AU) = RPE × duration in minutes`, mapped onto a five-step `Band` (light / moderate / substantial / demanding / veryDemanding) with a `requiresRecovery` flag on the top two. **Purely derived** — `rpe` and `movingTime` already live on `ActivityRecord`, so no `@Model` change and no schema migration (§2.1 not triggered). Invalid input (missing RPE, the `ignoredRPESentinel` 0, out-of-scale values, non-positive or sub-30-second durations) yields `nil` rather than a misleading 0 AU. Band cut-offs are heuristic, tuned for one property: long-easy must outrank short-hard. 11 unit tests in `SessionLoadCalculatorTests`, including the 30 km case and the explicit `longEasy > shortHard` ordering assert.
* **✅ 74.2 — Session load in the coach context.** `LastWorkoutContextFormatter` gained `durationSeconds:` and appends `Duration: N min. Session load: N AU (band, sRPE R x N min).` — arithmetic spelled out rather than left to the model. For the two `requiresRecovery` bands it adds an explicit counter-statement ("RPE measures intensity only — the volume makes this a heavy training stimulus…"), because the observed failure was the model concluding the opposite from a low RPE. `CoachContextStore.cacheLastWorkoutFeedback` passes `movingTime` through; both `DashboardView` call sites (check-in callback + `onAppear` priming) supply it. Four new formatter tests.
* **✅ 74.3 — `systemInstruction` rule rewritten.** *RPE DISCREPANCY* → **SESSION LOAD vs RPE**. The "good day" clause now requires a low RPE **and** a light/moderate load (<300 AU); a `demanding`/`veryDemanding` load is treated as a hard session regardless of the reported RPE, with the instruction to name the volume as the reason. The low-TRIMP-with-RPE-≥8 overtraining warning is unchanged. `CoachPromptAssembler`'s inline hint follows suit.
* **✅ 74.4 — Per-workout chat (Epic #70) carries the load too.** `WorkoutChatViewModel.buildPrompt` extends the `- Check-in:` line with `session load N AU (band)`; `WorkoutChatScopeInstruction` gained a "READING THE DATA" paragraph defining the term, since that chat runs on its own system instruction. Pinned in `WorkoutChatViewModelTests`.
* **✅ 74.5 — Session-RPE framing in the check-in card.** `PostWorkoutCheckinCard` now asks *"Hoe zwaar was deze training als geheel?"* with a hint that the duration counts as much as the pace, and option descriptions stripped of the talk test ("Kostte me weinig, was zo weer klaar" / "Prima te doen — stevig, maar ik had nog wat over" / "Zwaar — vroeg veel van me, dit hakt erin"). The stored `Int` values are unchanged, so `SessionType.expectedRPERange`, the four RPE buckets and the overtraining check keep working untouched. Six new `Localizable.xcstrings` keys (NL source + EN/DE/ES), five replaced keys removed, catalog normalised.

**Merged via PR #362. Effort:** ~1 session (5 stories; 15 new unit tests + 1 extended assert, unit suite 1301 green, SwiftLint `--strict` clean, `check-doc-consistency.py` clean).

**Deliberately out of scope:** surfacing the computed load back to the user in the UI (e.g. "je gaf 'Makkelijk' op 3u02 — dat telt als een zware prikkel, 364 AU"). It would close the loop visibly and is the obvious follow-up, but it is a new surface rather than a fix to the broken one; parked as a candidate for a later epic.
