...
* **✅ 53.2 — REST-clients voor OpenAI, Claude & Mistral:** `OpenAICompatibleModelClient` (bedient OpenAI + Mistral via `/v1/chat/completions` met `Authorization: Bearer`; één gedeelde client i.p.v. twee near-identieke types — DRY) en `AnthropicModelClient` (`/v1/messages`, `x-api-key` + `anthropic-version`, JSON via assistant-prefill `{`). System-instructie-plaatsing en JSON-mode per provider, base64-vision-parts, fout-mapping naar `AIProviderError` via `AIProviderHTTP.validate` (429/503/529 → `.overloaded`, 401/403 → `.authenticationFailed`). Transport-fouten bubbelen als `URLError` door naar de bestaande mapper. Timeout via `URLRequest.timeoutInterval`. 24 unit-tests in `AIModelClientTests` (URLProtocol-mock, geen live calls). **Mistral-enum-case** (`AIProvider.mistral`) vooruitgehaald uit 53.3 zodat de factory-switch compleet is; key-opslag-per-provider + UI blijven in 53.3/53.6.
* **⏳ 53.3 — `AIProvider.mistral` + per-provider sleutelopslag:** `case mistral` toevoegen aan de enum (displayName "Mistral", keyPlaceholder, `getKeyURL` → console.mistral.ai). `isSupported` retourneert nu `true` voor alle vier. `UserAPIKeyStore` omzetten naar keyed opslag per provider + eenmalige migratie van de legacy single-key naar de `gemini`-slot. Settings/Onboarding lezen/schrijven de sleutel van de **actieve** provider.
* **⏳ 53.4 — Model-selectie per provider:** `AIModelCatalog` is nu Gemini-only (Cloudflare Worker `/ai/models`). Voor OpenAI/Anthropic/Mistral een gecureerde statische catalogus per provider (modellijsten wijzigen traag; geen Worker-roundtrip nodig). `AIModelAppStorageKey` uitbreiden naar per-provider primair/fallback-keuze, of de keys prefixen met de provider. Default primair/fallback per provider hardcoden (bv. OpenAI `gpt-4o-mini` + `gpt-4o`, Claude `claude-haiku` + `claude-sonnet`, Mistral `mistral-small` + `mistral-large`) — exacte modelnamen valideren bij implementatie.
* **⏳ 53.5 — Multi-provider sleutel-validatie:** `APIKeyValidator` is Gemini-only (hardcoded `gemini-flash-latest`-ping). Uitbreiden met een lichte validatie-ping per provider (goedkoopste model, minimale prompt) en de fout-classificatie (`classify(_:)`) generaliseren naar `AIProviderError`. De "Test sleutel"-knop in Settings werkt dan voor elke provider.
* **⏳ 53.6 — Settings-UI uitbreiding:** `AIProviderSettingsView` toont nu een Gemini-only dual-picker (Epic #35). Provider-picker met alle vier opties; per geselecteerde provider de juiste `keyPlaceholder`, `getKeyURL`-link, sleutelveld (uit de per-provider Keychain-slot) en model-picker (Gemini = live Worker-catalogus, overige = statische catalogus). `aiCoachConnectionSubtitle` (Epic #43) toont de actieve provider + model.
* **⏳ 53.7 — Onboarding-flow:** stap "Jouw AI" (`OnboardingView` + `AIProviderPrivacyContent`) laat nu alleen provider kiezen; sleutel-invoer is uitgesteld naar Settings. Provider-keuze koppelen aan de juiste `getKeyURL` + (optioneel) inline sleutel-invoerkaart zodat een nieuwe gebruiker direct met zijn provider naar keuze kan starten. Bevestigen dat "Sla over, doe later" + `NoAPIKeyView`-lege-staat blijven werken ongeacht provider.
...
* **⏳ 53.9 — Testsuite + docs:** `MockGenerativeModel` en `UITestMockGenerativeModel` aanpassen aan de SDK-vrije protocol-signatuur. Per-client unit-tests (request-body-vorm, system-instructie-plaatsing, JSON-mode, fout-mapping) met gemockte `URLProtocol` — géén live API-calls. Validator-tests per provider. Migratie-test voor de keyed key-store (legacy-sleutel → gemini-slot). UI-test voor providerwissel in Settings via een nieuw launch-arg. `ARCHITECTURE.md §3` bijwerken met de nieuwe abstractie + per-provider-verschillen, en `architecture.json`/`.html` syncen (nieuwe `AIModelFactory` + drie clients als modules; `docRevision` bumpen) conform [CLAUDE.md §7](../CLAUDE.md#architectuur-visualisatie-afgeleide-artefacten).
...
**Status:** 🔄 — **Sprint A (53.1 + 53.2 + 53.8) afgerond** op branch `feature/epic-53-multi-provider-byok`: provider-agnostische client-laag staat, alle 895 unit-tests groen (+24 nieuwe client-tests). Runtime-gedrag voor Gemini is ongewijzigd (provider blijft `gemini` tot de UI in sprint C de andere selecteerbaar maakt). Sprint B (53.3 + 53.4 + 53.5 — per-provider key-opslag + model-catalogus + validatie) en sprint C (53.6 + 53.7 + 53.9 — Settings-UI + onboarding + docs-sync) volgen.
