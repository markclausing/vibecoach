...
|---|---|---|
| ⏳ **#63** | CI pipeline extensions — TestFlight deploy (63.1, **maintainer wants this**), snapshot tests + dep scan (63.2+63.3 coupled), perf baselines (63.4) | 63.1: as soon as the maintainer's App Store Connect prerequisites are in place; others per-story |
...
- **Concurrency (62.6 ✅, guarded by 63.5 ✅)** — `SWIFT_STRICT_CONCURRENCY = complete` is set **project-wide**; in Swift 5 language mode the checker only *warns*, so the build stays green regardless. Measured during 63.5 (normalised + deduplicated, clean build): **36** diagnostics in the app target (52 before excluding SwiftData `#Predicate` macro noise), 109 in unit tests, 54 in UI tests. The app-target 36 are frozen in `ci/concurrency-baseline.txt`; the test-target ones are XCTest/XCUITest isolation boilerplate and deliberately out of scope.
...
* **✅ 63.5 — Concurrency regression guard (was 46.B5; story redesigned July 2026):** the original text ("a matrix cell that compiles with `SWIFT_STRICT_CONCURRENCY=complete`") is **obsolete as written**: 62.6 already set `complete` project-wide, and in Swift 5 language mode it emits **warnings** (~748 across app+tests) — a plain matrix cell shows nothing new, and warnings-as-errors would fail on all existing debt at once. Redesign as a **baseline-count guard**:
  ...
  **Delivered as designed**, with three findings from building it against the real log. (a) The `~748` figure in the original grounding was never measured — a clean build yields **52** normalised diagnostics in the app target (109 unit-test + 54 UI-test ones are excluded as XCTest/XCUITest boilerplate: `XCUIApplication` is `@MainActor`, the test methods are not, so including them would drown the signal and redden every new UI test). (b) Diagnostics are **deduplicated, not counted** — Swift emits the same warning from both the `SwiftEmitModule` and `SwiftCompile` phases, and which ones get double-reported depends on whether they sit in a declaration signature or a body, so counts would wobble without any code change. (c) `#Predicate` diagnostics (from `@Query`) report `macro expansion #Predicate` as their file and carry no path; they are attributed to the call site named by the compiler's `expanded code originates here` note, otherwise every `#Predicate` in the project would collapse into one fileless entry. (d) **The baseline belongs to one toolchain.** The first CI run went red and proved it: the runner had Xcode 26.6 against Xcode 27.0 locally, and Swift had reworded one diagnostic ("data races between code in the current *task*" → "…in the current *isolation context*") while changing the macro-diagnostic format entirely. There is no compiler-independent key to normalise to — the `[#MutableGlobalVariable]` group tags only exist from Xcode 27. Resolution: macro diagnostics are excluded outright (unactionable Apple-macro noise; after that the two toolchains agree on **35 of 36** entries), the guard job pins an exact Xcode so an image rotation cannot silently change the compiler, the baseline records which Xcode froze it, and a mismatch is reported as a toolchain difference instead of as new debt. The script also rejects a log with **zero** diagnostics while the baseline is populated — that means an incremental build (warnings are only re-emitted for recompiled files), which would otherwise pass the guard for entirely the wrong reason. Verified end-to-end on a clean build: a deliberate `static var` violation fails the job naming file + message, a shrinking baseline passes with a re-freeze nudge, an incremental log errors. **Acceptance met:** a PR introducing a new actor-isolation/Sendable warning fails the job naming the diagnostic; removing warnings never fails; `permissions:` stays `contents: read`. **Endgame** remains the deferred Swift 6 language-mode migration (`SWIFT_VERSION = 6.0` turns all of these into hard errors — its own multi-day epic); this guard stops the debt growing until then. **Merged via PR #361. Effort:** ~2h.
...
**Suggested order (remaining):** 63.1 first (maintainer wish confirmed; blocked only on the one-off Apple prerequisites), then 63.2+63.3 as a pair when a visual regression bites, 63.4 opportunistically.

**Pickup trigger (epic-level):** the TestFlight wish (63.1, active), or a concrete regression the current pipeline misses (63.2/63.4).
...
- ✅ **#74** Session load — RPE is intensity only; the coach now weighs sRPE × duration (Foster AU), and the check-in asks how heavy the session was *as a whole*
