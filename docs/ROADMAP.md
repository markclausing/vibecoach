...
* **🔄 Story 61.8 — Migratie weg van de niet-onderhouden AI-SDK (afgesplitst uit 61.5).** Vervangt `google/generative-ai-swift` (deprecated upstream, CWE-1104/L-3) door `GeminiRestClient` — een directe URLSession REST-client tegen de Gemini v1beta `/generateContent`-endpoint, hetzelfde patroon als de OpenAI/Anthropic/Mistral-clients. Auth via `x-goog-api-key`-header (niet query-param, zodat de sleutel uit access-logs blijft). Het enige externe Swift-pakket is daarmee verwijderd. `GenerateContentError`-tak verwijderd uit mapper, validator en `isOverload()`.

**Status 61.1–61.3:** ✅ PR #316. **Status 61.4:** ✅ PR #317. **Status 61.6:** ✅ PR #318. **Status 61.5:** ✅ PR #319. **Status 61.7:** ✅ PR #320 + bugfix PR #322. **Status 61.8:** 🔄 PR #321 in review.

**Pickup-trigger:** opvolging van de security-review; stories 61.1–61.7 gedaan, 61.8 (SDK-migratie) in review. **Doc-discipline:** elke story die een building-block-type toevoegt of wijzigt synct `architecture.json` + `.html` (§7); `CLAUDE.md` alleen aanraken als een story een permanent patroon vastlegt.
