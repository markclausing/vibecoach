...
* **⏳ Story 61.7 — PHI-context-caches naar beschermde opslag (vervolg op 61.3).** Verplaats de prompt-context-caches uit cleartext `UserDefaults`/`@AppStorage` naar de file-protected SwiftData-store (of een app-group-bestand met `NSFileProtectionComplete`), zodat de data niet alleen bij ontkoppeling wordt gewist maar überhaupt niet meer in onbeschermde voorkeuren staat. Grotere refactor; bewust losgekoppeld van de kleine 61.3-ingreep.
...
**Status 61.1–61.3:** ✅ gemerged via PR #316 (data-at-rest + observability). **Status 61.4:** ✅ PR #317 (AI-coach defense-in-depth). **Status 61.6:** ✅ PR #318 (locatie-privacy). **Status 61.5:** ✅ gemerged via PR #319 (build- & robustness-hygiëne). Resterend: 61.7 (cache-relocatie) + 61.8 (SDK-migratie).

**Pickup-trigger:** opvolging van de security-review; alle kleine hardening-stories (61.1–61.6) gedaan, resterend de twee grotere: 61.7 (cache-relocatie) + 61.8 (SDK-migratie). **Doc-discipline:** elke story die een building-block-type toevoegt of wijzigt synct `architecture.json` + `.html` (§7); `CLAUDE.md` alleen aanraken als een story een permanent patroon vastlegt (bijv. een nieuwe validator-helper).
