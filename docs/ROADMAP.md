...
**Status:** 🔄 — **Sprint A (53.1 + 53.2 + 53.8) afgerond** op branch `feature/epic-53-multi-provider-byok`: provider-agnostische client-laag staat, alle 895 unit-tests groen (+16 nieuwe client-tests). Runtime-gedrag voor Gemini is ongewijzigd (provider blijft `gemini` tot de UI in sprint C de andere selecteerbaar maakt). Sprint B (53.3 + 53.4 + 53.5 — per-provider key-opslag + model-catalogus + validatie) en sprint C (53.6 + 53.7 + 53.9 — Settings-UI + onboarding + docs-sync) volgen.
