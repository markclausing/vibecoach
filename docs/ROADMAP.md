...
Idee voor een toekomstige Epic — nog niet uitgewerkt. De huidige `README.md` is **ontwikkelaar-gericht**: status, tech stack, setup. Een nieuwe bezoeker (potentiële gebruiker, of iemand die de repo via GitHub vindt) krijgt geen gevoel van *wat de app voor hém doet* en *hoe die eruitziet*. Doel van deze Epic: de README herstructureren naar een echte productpagina die bovenaan de gebruiker aanspreekt en pas daarna de techniek behandelt — zonder de bestaande doc-discipline (§7) te breken (README blijft "wat is dit + waar staat de info"; detail blijft in ROADMAP/ARCHITECTURE).

**Beoogde opbouw (drie lagen):**

1. **Voor de gebruiker — features in beeld.** Per major feature een **screenshot** + korte omschrijving + het **persoonlijke voordeel** ("wat levert het jóu op"). Kandidaat-features: Vibe Score / readiness, proactieve coaching (Management by Exception), AI-chatcoach (BYOK multi-provider), workout-deep-dive (patroondetectie: decoupling/drift/cadence/HR-recovery), dual-source sync (HealthKit + Strava), meerdaagse events + per-etappe-weer, meertaligheid (NL/EN/DE/ES), privacy-first (data on-device). Vorm: vergelijkingstabel of feature-blokken met afbeelding links/rechts.
2. **Technisch — voor de geïnteresseerde lezer.** Beknopte tech-samenvatting (stack, architectuurprincipes, CI) met **doorverwijzingen** naar de bestaande bronnen i.p.v. duplicatie: [`docs/ARCHITECTURE.md`](ARCHITECTURE.md), deze `ROADMAP.md`, [`CLAUDE.md`](../CLAUDE.md), de interactieve [`docs/architecture/architecture.html`](architecture/architecture.html). Eén screen, geen herhaling van wat al elders staat (§7: geen overlap).
3. **Zelf bouwen & draaien.** Een duidelijke stap-voor-stap voor een nieuwe ontwikkelaar: vereisten (macOS + Xcode 16, iOS 17+ device aangeraden voor HealthKit), `Secrets.swift` uit het template, BYOK-AI-sleutel toevoegen in Settings, optionele Strava-proxy ([vibecoach-proxy](https://github.com/markclausing/vibecoach-proxy)), build & run (Cmd+R), en hoe je de testsuite draait. Bouwt voort op de huidige "Installation & Setup"-sectie maar maakt 'm compleet.

**Open punten:** (a) waar leven de screenshots — `docs/screenshots/` in de repo (versiebeheer, vergroot repo-grootte) vs. externe hosting; (b) screenshots zijn **per taal** en verouderen bij UI-wijzigingen → onderhoudslast, mogelijk beperken tot EN + een vaste set "hero"-schermen; (c) hoeveel productmarketing-toon past bij een persoonlijk project; (d) consistentie met de README-scoperegel uit §7 (README kort houden) — een lange showcase kan botsen met "Current Status < 1 screen", dus misschien de showcase als aparte `docs/`-pagina met een korte teaser + link vanuit de README. **Pickup-trigger:** de app gaat naar TestFlight/bredere distributie, óf de maintainer wil de repo presentabel maken voor externe bezoekers.
