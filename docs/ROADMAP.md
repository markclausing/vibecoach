...
* **🔄 51.F — Data syncen (deels):**
  - **✅ F1 + F2 + F5:** `SyncStatusStore` houdt per data-source de laatste succes-timestamp en de laatste fout-categorie bij; `SyncBannerStateBuilder` (pure-Swift) bepaalt op basis van een snapshot welke banner getoond wordt — prioriteit offline > rate-limited > error > nil zodat één centrale `SyncStatusBanner` op het Dashboard alle drie de meldingen consistent rendert. F2: `FitnessDataError.rateLimited(retryAfter:)` + `StravaRateLimitParser` extraheert het `Retry-After`-tijdstip (delta-seconds én HTTP-datum, fallback 15 min) en `StravaRateLimitStore` persisteert de cooldown over app-launches heen zodat een retry-storm vlak na launch onmogelijk wordt. F5: `NetworkReachabilityMonitor` (NWPathMonitor-wrapper, singleton) zet de offline-state. `.missingToken` is bewust uitgefilterd — gebruikers zonder Strava-koppeling krijgen geen banner. 24 unit-tests (11 parser + 13 banner-builder) + 4 nieuwe FitnessDataService 429-tests. Gemerged via PR #272.
  - **🔄 F3 + F4 + F6:** F6 captive-portal-detectie via `CaptivePortalDetector` (pure-Swift content-type + HTML-prefix-check) ingebouwd in `FitnessDataService.validateHTTPResponse` en `HistoricalWeatherService.fetchWeather`, plus een actieve probe naar `captive.apple.com/hotspot-detect.html` die `NetworkReachabilityMonitor` triggert na elke offline→online-transitie. `SyncBannerState.captivePortal` als nieuwe staat in de banner-priority (offline > captive-portal > rate-limited > error). F4 weer-fout-retry: `WeatherRetryStore` (UserDefaults-dict `[activityID: failedAt]`, geen schema-migratie — passend bij verrijking-niet-kern karakter van weer-data); `HistoricalWeatherService.enrichRecord` respecteert nu een 1u-retry-cooldown en wist de marker bij succes zodat een latere sync zonder portal de bestaande DB-records aanvult via de smartInsert-merge. F3 per-type HK-permissie-zichtbaarheid: `HealthKitPermissionTypes.criticalTypeStatuses(in:)` levert workouts/hartslag/HRV/actieve-energie per type; `HealthKitSyncStatusEvaluator.criticalDeniedTypes` filtert alleen expliciet geweigerde types (`.notDetermined` valt buiten — verse installs krijgen geen rode banner). Dashboard toont een aparte oranje banner *"Apple Health-toegang ontbreekt"* met opsomming van geweigerde types; `HealthKitPermissionsSection` in Settings rendert per-type status-rijen (groen/grijs/rood) + deeplink naar Instellingen. 19 nieuwe unit-tests (12 detector + 9 retry-store + 3 evaluator-gap).
