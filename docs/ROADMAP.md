...
* **🔄 Story 61.7 — PHI-context-caches naar beschermde opslag (vervolg op 61.3).** Nieuw `@Model CoachContextCache` (SwiftData-singleton, SchemaV6) vervangt 17 `@AppStorage`-properties in `ChatViewModel` — HRV/slaap-scores, per-lichaamsgebied pijnscores, 14-daagse workout-TRIMP/HR-geschiedenis, voedingsdata e.d. — die als plaintext in `Library/Preferences` stonden. Door ze naar de file-protected SwiftData-store te verplaatsen (lightweight migratie V5→V6) genieten ze `NSFileProtectionCompleteUnlessOpen` zoals alle andere `@Model`-types. `PHIContextCache` beheert nu alleen de resterende UserDefaults-sleutels; `StravaAuthService.logout(modelContext:)` wist beide kanten. `AppTabHostView` injecteert de `ModelContext` via `chatViewModel.configure(with:)`. 3 migratietests (file-backed, §2.1).
* **⏳ Story 61.8 — Migratie weg van de niet-onderhouden AI-SDK (afgesplitst uit 61.5).** Plan de overstap van `google/generative-ai-swift` (deprecated upstream) naar de onderhouden SDK; revisie-pin handhaven en upstream-advisories volgen tot dan (L-3). Grotere, op zichzelf staande ingreep die de hele AI-call-laag raakt — bewust apart gehouden van de kleine build-hygiëne-fixes in 61.5.

**Status 61.1–61.3:** ✅ gemerged via PR #316 (data-at-rest + observability). **Status 61.4:** ✅ PR #317 (AI-coach defense-in-depth). **Status 61.6:** ✅ PR #318 (locatie-privacy). **Status 61.5:** ✅ gemerged via PR #319 (build- & robustness-hygiëne). **Status 61.7:** 🔄 in review. Resterend: 61.8 (SDK-migratie).

**Pickup-trigger:** opvolging van de security-review; kleine hardening-stories (61.1–61.6) gedaan, 61.7 in review, resterend 61.8 (SDK-migratie). **Doc-discipline:** elke story die een building-block-type toevoegt of wijzigt synct `architecture.json` + `.html` (§7); `CLAUDE.md` alleen aanraken als een story een permanent patroon vastlegt (bijv. een nieuwe validator-helper).
