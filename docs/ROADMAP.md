...
1. **30 juni 2026 — abonnementsplicht (URGENT, maintainer-actie, geen code).** Standard Tier-developers hebben vanaf deze datum een Strava-abonnement nodig om de API te blijven gebruiken; actieve developers krijgen 3 maanden gratis. Zonder actie valt de Strava-sync uit. **Let op het onderscheid:** de eis is een **betaald Strava-*abonnement*** (premium membership), niet enkel een (gratis) Strava-*account*. De maintainer gebruikt de app puur persoonlijk = **1 atleet**, wat ruim binnen de Standard Tier-limiet van 10 atleten self-service valt — de tier zelf is dus geen probleem; alleen de subscription-eis telt. → **Bevestigen of het account al een betaald abonnement heeft; zo niet: de 3 maanden gratis claimen, daarna betalen of accepteren dat Strava-sync wegvalt en de app HealthKit-only doordraait (zie punt onder Open punten).**
