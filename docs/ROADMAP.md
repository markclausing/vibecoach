...
- **SPM surface** — `xcshareddata/swiftpm/Package.resolved` still pins **only** `generative-ai-swift` 0.5.6, which Epic #61.8 (PR #321) was supposed to remove. Either a stale pin to clean up or an unmerged remnant → the dependency-scan story (63.3) has near-zero surface until that is resolved.
...
* **⏳ 63.3 — Dependency vulnerability scan (was 46.B3):** GitHub's `dependency-review-action` on PRs that touch `Package.swift`/`Package.resolved`, comparing new transitive deps against the GitHub Advisory Database. **Precondition:** first resolve the stale `generative-ai-swift` pin noted above — with zero real SPM deps the action has nothing to review, so this is only meaningful once the project genuinely takes on a third-party package. Acceptance: a PR that adds a vulnerable dep is flagged; a clean PR passes silently. Effort: ~30 min once there is a manifest worth scanning. **Pickup trigger:** adding the first genuine third-party SPM dependency.
...
* **⏳ 63.6 — Semver via `release-please` + git-tag-based `MARKETING_VERSION` (was 46.B6):** usable independently of 63.1 (a tag + GitHub Release have standalone value as release history). Three sub-steps: (1) a `googleapis/release-please-action` workflow on main; the bot maintains a Release PR that, on merge, creates the git tag (`v1.2.3`) + a GitHub Release with changelog, with patch/minor/major derived from Conventional Commits (`fix:` / `feat:` / `feat!:`); (2) a Run Script Build Phase that sets `CFBundleShortVersionString` at build time from `git describe --tags --abbrev=0`, parallel to the existing `CFBundleVersion` script — one source of truth (the tag), no `MARKETING_VERSION` mutation in `project.pbxproj` (keeps its `skip-worktree` flag, §9); (3) formalise Conventional Commits in CLAUDE.md §8 as a hard rule (already followed in practice, but release-please relies on it). Note: release-please needs `contents: write` + `pull-requests: write` on **its own** workflow only — keep the test workflow least-privilege. Acceptance: merging a `feat:`/`fix:` commit to main opens/updates a Release PR; merging that PR tags + releases; the app version reflects the latest tag. Effort: ~2–3h one-off. **Pickup trigger:** the first real release, or wanting explicit release history before it.

**Suggested order:** 63.6 first (release history has value immediately and unblocks 63.1's versioning), then 63.1 (TestFlight, once you do a release), 63.5 (right after 62.6 lands), 63.2 (when a visual regression bites), and 63.3 + 63.4 opportunistically (both low-value until their precondition/need exists).
