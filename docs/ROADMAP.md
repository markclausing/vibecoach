...
**Status 61.1–61.3:** ✅ gemerged via PR #316 (de hoogste-prioriteit data-at-rest + observability-cluster). **Status 61.4:** ✅ gemerged via PR #317 (defense-in-depth rond de AI-coach). **Status 61.6:** ✅ gemerged via PR #318 (locatie-privacy). Resterend: 61.5 (build/afhankelijkheid-hygiëne) + 61.7 (cache-relocatie).

**Pickup-trigger:** opvolging van de security-review; 61.1–61.4 + 61.6 gedaan, resterend 61.5 + 61.7. **Doc-discipline:** elke story die een building-block-type toevoegt of wijzigt synct `architecture.json` + `.html` (§7); `CLAUDE.md` alleen aanraken als een story een permanent patroon vastlegt (bijv. een nieuwe validator-helper).
