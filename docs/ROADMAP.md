...
* **🔄 51.A — Coach-gesprek (deels):**
  - **🔄 A1 + A5 (deelimplementatie):** `ChatScopeInstruction` plakt een expliciete scope-restrictie bovenaan de system-prompt (coach weigert off-topic-vragen met vaste framing); `ChatErrorMessageMapper` (pure-Swift) vervangt de generieke "tijdelijk probleem"-melding door specifieke teksten per fout-categorie — offline / timeout / DNS / cancelled / safety-block / invalid-key / overbelast / generiek. 17 unit-tests.
  - **⏳ A2 / A3 / A4 / A6:** provider-switch flow, lange-gesprek-trimming, paste-limit, request-cancellation — losse PR.
