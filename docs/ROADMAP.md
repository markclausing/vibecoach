...
* **✅ Story 61.7 — PHI-context-caches naar beschermde opslag (vervolg op 61.3).** Nieuw `@Model CoachContextCache` (SwiftData-singleton, SchemaV6) vervangt 17 `@AppStorage`-properties in `ChatViewModel` — HRV/slaap-scores, per-lichaamsgebied pijnscores, 14-daagse workout-TRIMP/HR-geschiedenis, voedingsdata e.d. — die als plaintext in `Library/Preferences` stonden. Door ze naar de file-protected SwiftData-store te verplaatsen (lightweight migratie V5→V6) genieten ze `NSFileProtectionCompleteUnlessOpen` zoals alle andere `@Model`-types. `PHIContextCache` beheert nu alleen de resterende UserDefaults-sleutels; `StravaAuthService.logout(modelContext:)` wist beide kanten. `AppTabHostView` injecteert de `ModelContext` via `chatViewModel.configure(with:)`. 3 migratietests (file-backed, §2.1). **Gemerged via PR #320.**
* **🔄 Story 61.8 — Migratie weg van de niet-onderhouden AI-SDK (afgesplitst uit 61.5).** Vervangt `google/generative-ai-swift` (deprecated upstream, CWE-1104/L-3) door een directe REST-client `GeminiRestClient` die de Gemini v1beta `/generateContent`-endpoint rechtstreeks via URLSession aanspreekt — het enige externe pakket is daarmee verwijderd. Auth: `?key=` query-param. JSON-mode: `generationConfig.response_mime_type`. Images: `inline_data` parts. Custom `validateGemini()` mapt HTTP 400 `INVALID_ARGUMENT` op `AIProviderError.authenticationFailed` (Google geeft geen 401 terug bij een slechte sleutel). `GenerateContentError`-tak verwijderd uit `ChatErrorMessageMapper`, `APIKeyValidator` en `isOverload()`. `AIModelClientTests.swift` bijgewerkt: `RealGenerativeModel` → `GeminiRestClient`.

**Status 61.1–61.3:** ✅ gemerged via PR #316 (data-at-rest + observability). **Status 61.4:** ✅ PR #317 (AI-coach defense-in-depth). **Status 61.6:** ✅ PR #318 (locatie-privacy). **Status 61.5:** ✅ gemerged via PR #319 (build- & robustness-hygiëne). **Status 61.7:** ✅ PR #320 (PHI-caches naar SwiftData). **Status 61.8:** 🔄 in review.

**Pickup-trigger:** opvolging van de security-review; kleine hardening-stories (61.1–61.7) gedaan, 61.8 (SDK-migratie) in review. **Doc-discipline:** elke story die een building-block-type toevoegt of wijzigt synct `architecture.json` + `.html` (§7); `CLAUDE.md` alleen aanraken als een story een permanent patroon vastlegt (bijv. een nieuwe validator-helper).
