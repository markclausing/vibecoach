...
* **✅ 46.1 — Splits `ios-tests.yml` in `unit-tests` + `ui-tests` jobs:** Twee aparte jobs op `macos-latest`. `unit-tests` draait `xcodebuild test` met `-only-testing:AIFitnessCoachTests`, `-enableCodeCoverage YES` en `-resultBundlePath UnitTests.xcresult`; bundle wordt geüpload via `actions/upload-artifact@v4` (`if: always()`, 7 dagen retentie) zodat zowel debugging-bij-falen als de coverage-job toegang heeft. `ui-tests` heeft `needs: unit-tests` en `-only-testing:AIFitnessCoachUITests`. Tradeoff: 2× macOS-runner-tijd per push, maar UI-falen kan geen unit-test-feedback meer vertragen. Tevens opgeruimd: duplicate `Setup Secrets`-step die op case-insensitive macOS toevallig werkte met verkeerde-casing pad (`Secrets-Template.swift`); `actions/checkout@v3` → `@v4` voor consistentie met `codeql.yml`. **Bijvangst:** UI-tests draaiden voorheen helemaal niet op CI — `AIFitnessCoachUITests` had `IPHONEOS_DEPLOYMENT_TARGET = 26.2` (Xcode-default, nooit verlaagd) waardoor `xcodebuild test` zonder `-only-testing` ze stilletjes oversloeg op een 18.x-simulator. Verlaagd naar 18.0 in `project.pbxproj` (matcht main app + unit tests; geen `@available iOS 26`-call sites). **UI-tests-job staat op `continue-on-error: true`:** XCUITest op GitHub `macos-latest` runners is structureel flaky met `ipc/mig server died` (Mach-308) op xctrunner-launch. Poging A (`xcrun simctl bootstatus -b` + `CODE_SIGNING_ALLOWED=NO`) bracht failures van 7→3 terug maar niet naar 0. UI-tests draaien daarom als signaal-only; harde validatie gebeurt on-device conform `feedback_epic_pr_workflow`. Bij toekomstige stabilisatie van Apple's runner-image (Xcode 17+) deze flag verwijderen.
* **⏳ 46.2 — Parallele `lint`-job (SwiftLint) — uitgesteld naar aparte cleanup-PR:** SwiftLint dry-run (default-rules op `AIFitnessCoach/`, v0.63.2) gaf **938 violations waarvan 211 errors** (mei 2026) — exact het scenario dat de oorspronkelijke risico-noot voorzag. **0 high-signal violations** (`force_unwrapping`, `force_cast`, `force_try`) — codebase is fundamenteel veilig, het is puur stijl-werk (line-length, comma/colon-spacing, identifier-namen). Direct integreren zou elke PR blokkeren. Aanpak voor later: eerst een `chore/swiftlint-cleanup`-PR die `.swiftlint.yml` aanlegt met de meest impactvolle regels (line-length, opening-brace, switch-case-alignment) en de bestaande violations oplost; daarna deze 46.2 als `lint`-job-PR.
...
* **🔄 46.4 — UI Tests CI-flakiness root-cause-onderzoek:** Tijdens 46.1 bleek dat 3 UI-tests deterministisch falen op de runner (`testCoachMemory`, `testNavigateToCoachTab`, `testNavigateToSettingsTab`) en 4 vlokken random. Het log toont `Clone 2 of iPhone 16 Pro` — `AIFitnessCoachUITests.xctest` heeft `parallelizable = "YES"` in het scheme, dus xcodebuild spawnt meerdere xctrunner-clones parallel. Hypothese: de clone-spawning + `ipc/mig server died` (Mach-308) zijn met elkaar verbonden. Investigatie-experiment op `ci/investigate-uitest-flakiness`: (1) `-parallel-testing-enabled NO` op CI-niveau (lokaal blijft scheme-config parallel), (2) `-resultBundlePath UITests.xcresult` met artifact-upload (per-test crash-reports + activity logs + screenshots), (3) `~/Library/Logs/CoreSimulator/` als artifact (SpringBoard / xctrunner crash-trails). Doel: ofwel hypothese bevestigen en de `continue-on-error: true` weer kunnen verwijderen, ofwel met data-onderbouwing besluiten dat UI-tests permanent signaal-only blijven. **Pickup:** wanneer de investigatie-PR rondkomt, 46.1-tekst over `continue-on-error` updaten met de uitkomst.
...
**Effort gerealiseerd (46.1 + 46.3):** ~3u (waarvan ~2u debugging UI-tests-launch op runner-environment). 46.2 is na dry-run uitgesteld naar een losse cleanup-PR (zie story-tekst). 46.4 loopt actief.

**Status:** 🔄 — 46.1 + 46.3 gemerged op `main` (PR #244). 46.2 staat geparkeerd in afwachting van een `chore/swiftlint-cleanup`-voorbereidings-PR. 46.4 root-cause-onderzoek loopt op `ci/investigate-uitest-flakiness`. Backlog-stories (46.B1–B6) blijven onaangeraakt.
