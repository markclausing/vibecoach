...
┌─ Concurrency Guard  (parallel, no needs)
├─ Unit Tests ────────┬─ UI Tests
└────────────────────┴─ Coverage Report
```
...
- **`Concurrency Guard`** — `scripts/check-concurrency-baseline.py` over a clean `xcodebuild build` log (story 63.5). `SWIFT_STRICT_CONCURRENCY = complete` is on project-wide (62.6), but under `SWIFT_VERSION = 5.0` the checker only *warns*, so nothing stopped the actor-isolation debt from growing. The job normalises every concurrency diagnostic to `<repo-relative file>: <message>` and diffs it against `ci/concurrency-baseline.txt`: **new entries fail**, disappearing entries pass with a nudge to re-freeze (`--update`). Line:column is deliberately dropped (an exact `file:line` baseline would "move" every diagnostic below an inserted line) and entries are deduplicated rather than counted (Swift emits the same diagnostic from both the `SwiftEmitModule` and `SwiftCompile` phases, so counts wobble for reasons unrelated to the code). Macro-expansion diagnostics — SwiftData's `#Predicate` inside `@Query` — are excluded: nobody can act on them (the code is Apple's macro) and the toolchains disagree on how to name them at all. Scope is the **app target only**: the test targets add ~163 diagnostics that are almost entirely XCTest/XCUITest boilerplate (`XCUIApplication` is `@MainActor`, the test methods are not), which would drown the signal and make every new UI test red. Baseline at introduction: **36 entries**.

  **The baseline belongs to one toolchain**, which is why this is the only job that pins an exact Xcode (`PINNED_XCODE`, currently `Xcode_26.6.app`) instead of following the `Xcode.app` symlink. There is no compiler-independent key to normalise to: Swift rewords diagnostics between releases (26.6 says "data races between code in the current *task*", 27 says "…in the current *isolation context*" for the same warning) and the `[#MutableGlobalVariable]` group tags only exist from 27 onwards. Without the pin, a GitHub runner-image rotation would silently change the compiler and turn the guard red on an unrelated PR. The baseline records the Xcode it was frozen with, and a mismatch is reported as a toolchain difference rather than as new debt — so a local run on a different Xcode is informative, not alarming. Bumping the pin means re-freezing the baseline from a CI log (the job uploads one as an artifact on failure; `relativise()` resolves the runner's checkout path, so `--update` works on a downloaded log). The endgame remains the deferred Swift 6 language-mode migration, where all of these become hard errors; this guard keeps the pile from growing until then.
...
### `Services/Physiology/PauseDetector.swift`
...
- **Explicit store protection.** `AIFitnessCoachApp.applyFileProtection(at:)` sets `ModelStoreRecovery.storeFileProtection` — `NSFileProtectionCompleteUntilFirstUserAuthentication` — on the SwiftData store + its WAL/SHM sidecars after every successful container init (which also migrates existing stores off the old class). The store is encrypted from boot until the first unlock, then stays openable while locked. Best-effort by design (§12): a failure is logged and never blocks launch; on the simulator it is effectively a no-op.
  - **Why not `.completeUnlessOpen` (Epic #61's original choice, reverted Sept 2026).** Its premise was that Engine A keeps writing to an *already-open* file. But iOS cold-launches the app in the background on a locked device (HealthKit delivery, `BGAppRefreshTask`), and `App.init` builds the `ModelContainer` on every launch — a closed `.completeUnlessOpen` file cannot be opened then. The init threw, the §12 fallback treated that as corruption and deleted the store, repeatedly wiping goals, preferences and workout-chat memory on the maintainer's device. The trade-off accepted: a locked-but-once-unlocked device can read the store (the same class iOS gives `UserDefaults` by default); credentials keep their stricter Keychain class.
- **Store recovery on a failed open.** `ModelStoreRecovery` (pure Foundation, file-backed tests) decides what `makeModelContainer()` does after the migration-plan init throws: store files that exist but cannot be `open(2)`ed (a launch before the first unlock) are **left untouched** and the launch runs on a temporary in-memory container; a readable store that still fails (migration failure, corruption) is **quarantined** — renamed to `default.store[-wal|-shm].quarantined-<UTC stamp>`, newest two sets kept — before a fresh store is built and `MigrationFallbackStore` flags the banner. Known limit: if a before-first-unlock background launch is later brought to the foreground in the same process, that session shows the empty in-memory store until the app is relaunched.
...
- **Location-in-transit privacy.** `CoordinatePrivacy` (Story 61.6) is the single source of truth for coordinate rounding: every weather request rounds the GPS/geocoded lat-lon to 0.1° (~11 km) before it leaves the device. `WeatherManager` (live forecast) and `OpenMeteoForecastClient` (per-stage) now round explicitly; `HistoricalWeatherService.roundForPrivacy` delegates to it. The privacy margin no longer depends on `CLLocationManager.desiredAccuracy` being coarse — a single future tightening of that setting can no longer silently start leaking street-level GPS.

---

## 17. AI-coach safety guardrails (Epic #61)

The coach's physiological safety limits (session length, week-over-week progression, intensity caps) and the trust boundary around synced free text previously lived **only in the prompt text**. Epic #61 Story 61.4 adds code-side defense-in-depth so a hallucinated or injection-steered model response cannot bypass them.
...
## 20. Per-workout chat with local memory (Epic #70)

The workout detail page (`WorkoutAnalysisView`) ends in a **"Discuss this workout"** section (`WorkoutChatSection`): a small chat anchored to that one workout where the user can tell the coach what the sensors can't know — how it felt, the route, a bad night's sleep. The design is **hybrid**: the full thread persists per workout, *and* the coach distils compact facts out of it that flow into plans and feedback.

**Persistence (SchemaV7, local-only).** Two `@Model`s keyed **by value** to `ActivityRecord.id` (no SwiftData relationship — activities re-sync from HK/Strava while the chat is local-only): `WorkoutChatEntry` (the thread) and `WorkoutChatFact` (a distilled fact with a type-safe `WorkoutFactCategory`: `feel` / `route` / `dayCondition`). Both share the `Symptom`/`UserPreference` data-loss class (§ Defensive App Init): the migration fallback wipes them, which is why `SchemaMigrationV6ToV7Tests` is file-backed.

**Scope guardrail + JSON contract.** `WorkoutChatScopeInstruction` is the narrower sibling of `ChatScopeInstruction`: allowed topics are exactly (a) this workout and (b) the user's condition today/this week; everything else — including plan questions that *are* in scope on the Coach tab — gets a redirect there, so plan changes keep flowing through one place. The same constant carries the response contract `{"reply", "workoutFacts": [{text, category}]}`; `WorkoutChatResponseParser` is the other side (unknown category → fact dropped at the front door; non-JSON → the raw text becomes the reply, facts empty — the coach reply is never lost to a JSON hiccup).
