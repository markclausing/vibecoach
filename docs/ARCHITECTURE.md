...
- **UI + onboarding** (sprint C): `AIProviderSettingsView` heeft per provider een sleutelveld (eigen Keychain-slot) + model-picker (Gemini = live Worker-catalogus, overige = statische `AIModelCatalog.builtIn(for:)`). De onboarding-stap "Jouw AI" laat alle vier providers kiezen met een `getKeyURL`-link.
