...
| **`docs/ROADMAP.md`** | **Live plan only:** the "Open werk" index (one row per open epic + pickup-trigger) + active/planned epics (🔄 / ⏳) + a short "Recent afgerond" pointer list. Completed epics move out to the archive. | Whoever (human or AI agent) wants a cheap, current "what's open" view | Keep it lean — open work + index; no full ✅-history here |
...
- **`docs/ROADMAP.md`** — Epic status ⏳ → 🔄 → ✅, tick off sub-stories, update the "Effort realised" + "Status" line. When starting a new Epic: add it to the "Open werk" index + an "Actief & gepland" section with rationale + sub-stories. **When an Epic is fully ✅: move its block to `docs/ROADMAP-archive.md` (keep original order), drop its "Open werk" index row, and add a one-line entry under "Recent afgerond".** This keeps the live ROADMAP cheap to load as context for humans and AI agents.
