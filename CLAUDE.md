...
6. **Repointing an earlier schema at a new snapshot breaks that schema's existing migration test.** Once e.g. `SchemaV5`/`V6`/`V7` point at the frozen `SchemaV7.FitnessGoal` instead of the live class, the existing `SchemaMigrationV4ToV5Tests` etc. crash at runtime ("Failed to cast model … to FitnessGoal") because they still fetch the **live** type out of a now-frozen intermediate container. Fix: in those tests fetch `FetchDescriptor<SchemaV<N>.FitnessGoal>()` and drop asserts on live-only computed properties. *Inserting* a live instance into a snapshot container is fine — only fetching breaks.
7. **A branch's own migration test is circular** — it seeds the V<N> store with the branch's *own* snapshot definition, so it cannot catch a snapshot-vs-live hash mismatch. To truly verify, seed a fixture store from a `git worktree` of `main` (the old live definition) and migrate that with the branch. Do this whenever the snapshot was hand-written rather than copied verbatim.
8. Always write a `SchemaMigrationV<N>To<M>Tests` with a **file-backed** seed store as a happy-path safety net. In-memory stores do not work for migration paths (there is no V<N> store file to start from). Test at least (1) that `FitnessGoal` + `UserPreference` records survive the migration (local-only loss risk) and (2) that new fields are writable after migration.
...
- **Strict-concurrency debt may shrink, never grow (Epic 63.5).** `SWIFT_STRICT_CONCURRENCY = complete` is on project-wide, but in Swift 5 language mode it only *warns* — so the `Concurrency Guard` job diffs the app target's concurrency diagnostics against `ci/concurrency-baseline.txt` and fails on anything new. Fix the isolation rather than widening the baseline: re-freezing (`python3 scripts/check-concurrency-baseline.py <build.log> --update`) is for when warnings **disappear**, or for a deliberate, explained exception in the PR body. The log must come from a **clean** build (fresh `-derivedDataPath`) — an incremental one re-emits nothing for untouched files, which the script rejects rather than reading as a clean slate. **The baseline belongs to the Xcode the job pins** (`PINNED_XCODE` in the workflow): Swift rewords diagnostics between releases, so a local run on a different Xcode reports a toolchain mismatch — that is informational, CI is the authority. Re-freeze from a downloaded CI log (the job uploads one on failure), not from a local build.
...
**Quick self-audit when in doubt or at epic close:** list the top-level type declarations you touched and confirm each building-block type has a matching `id` in `architecture.json` (`grep '"id":' docs/architecture/architecture.json`). A missing entry = drift to fix now. Registration itself is *not* machine-checkable (nothing can know which types are building blocks), so this audit stays manual — but the two mechanical invariants around it are enforced by the `Doc Consistency` CI job, see below.

**Automated backstop — run `python3 scripts/check-doc-consistency.py` before pushing.** The `Doc Consistency` job (`ios-tests.yml`, `ubuntu-latest`, parallel) fails the build on the two derived-artefact drifts that have actually happened: `meta.appVersion` diverging from `.release-please-manifest.json`, and `architecture.html` embedding a stale copy of `architecture.json`. It tolerates version lag while a release is in flight (the release-please PR + its merge commit) so that machine-generated PR never goes red, and fails hard everywhere else with a message naming the exact fix.
...
**Versioning.** `meta.appVersion` matches the **released** marketing version — read it from `.release-please-manifest.json` (identical to the newest `v*` git tag), never from `AIFitnessCoach/Info.plist`. The source plist's `CFBundleShortVersionString` is deliberately frozen (`2.0.0`) as a fallback for tag-less builds and never ships: the Build Phase stamps the real value from `git describe --tags` into the built app only. This rule used to point at the plist, which is exactly how `appVersion` came to read `2.0.0` while the app was shipping `2.4.0` — a number nobody could place. **When a release is cut** (the maintainer merges the release-please PR, §8.2), set `appVersion` to the new version and reset `docRevision` to `1`. `docRevision` counts architecture revisions **within one released version**, so "`X.Y.Z` rev 3" reads as "the architecture three revisions after the `X.Y.Z` release" — it may already describe code destined for the next release, which is intended. It gives AI agents and reviewers a clear anchor for whether a cached picture is still current. `meta.buildNumberSource` records how `CFBundleVersion` is determined (git commit count via a Build Phase script) — we do *not* mirror that dynamic number into the docs, otherwise every commit would trigger a doc bump.
