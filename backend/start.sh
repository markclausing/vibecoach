...
# Controleer of het .env bestand bestaat en laad de variabelen
if [[ -f .env ]]; then
    set -a
    source .env
    set +a
    echo "✅ Lokaal .env bestand succesvol geladen!"
else
    echo "⚠️ Waarschuwing: Geen .env bestand gevonden. Zorg dat je de juiste variabelen hebt ingesteld."
fi
