...
| **Gemini-flash-lite fallback + 45s timeout** (2026-04) | In de praktijk blijkt `gemini-flash-latest` tijdens piekuren vaak `internalError` (503/429) te retourneren, waardoor Dashboard pull-to-refresh 120s vastliep voordat de fout zichtbaar werd. Opgelost met een stille fallback naar `gemini-flash-lite-latest` (zelfde generatie, lichter model, vaker capaciteit) én een timeout-reductie van 120s → 45s. De fallback zit in `ChatViewModel.fetchAIResponse` én `AddGoalView.fetchAITargetTRIMP`. Andere fouten (invalid key, prompt blocked) slaan de fallback over en tonen direct de juiste melding. | Vaste retry-loop op hetzelfde model — verworpen omdat overbelasting meestal langer aanhoudt dan een paar seconden |
