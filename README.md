...
- 🗺️ Interactive architecture viewer (redesigned in Epic #66: starts as a plain-language story for non-programmers, then lets you drill down — map of all modules → per-module detail with builds-on/used-by navigation; a "How it's built" view visualises the dev workflow — agent collaboration, CI pipeline, branching & docs — added in Epic #67; mobile-friendly) → **[open rendered](https://htmlpreview.github.io/?https://github.com/markclausing/vibecoach/blob/main/docs/architecture/architecture.html)** (GitHub shows `.html` as source, so this opens it via htmlpreview) · [source](docs/architecture/architecture.html)
